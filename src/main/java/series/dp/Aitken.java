...
public final class Aitken extends SeriesAlgorithm {
	...
	public Aitken(final double tolerance, final int maxIters) {
	...
	public final double next(final double e, final double term) {
		...
		if (myIndex == 0) {
			...
			alpha = modait(alpha, ts, dts, myIndex - 1, -2.0);
		}
		xp = xx;
		if (myIndex >= 2) {
			for (int m = 1; m <= myIndex + 1; ++m) {
				final double xx = x[m - 1];
				xp = modait(xx, s, ds, m, alpha);
			}
		}
		++myIndex;
		if (status == 1) {
	...
	private double modait(final double xx, final double[][] s, final double[][] ds, final int n, final double theta) {
		status = 0;
		if (n != 1) {

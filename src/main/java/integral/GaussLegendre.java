...
public final class GaussLegendre extends Quadrature {
    ...
    private static double g8(final Function<? super Double, Double> fun, final double x, final double h) {
	final double fx1 = fun.apply(x - X1 * h) + fun.apply(x + X1 * h);
	final double fx2 = fun.apply(x - X2 * h) + fun.apply(x + X2 * h);
	final double fx3 = fun.apply(x - X3 * h) + fun.apply(x + X3 * h);
	final double fx4 = fun.apply(x - X4 * h) + fun.apply(x + X4 * h);

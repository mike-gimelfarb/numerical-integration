...
/**
 * An adaptive integrator obtained by replacing the Gauss-Kronrod rules in
 * QUADPACK with recursive monotone stable (RMS) formulas [1]. A translation of
 * a corresponding Fortran subroutine by Alan Miller.
 * 
 * <p>
 * References:
 * <ul>
 * <li>[1] Favati, Paola, Grazia Lotti, and Francesco Romani. "Algorithm 691:
 * Improving QUADPACK automatic integration routines." ACM Transactions on
 * Mathematical Software (TOMS) 17.2 (1991): 218-232.</li>
 * </ul>
 * </p>
 */
public final class RmsRule extends Quadrature {
    ...
    private static final double[] xx = { 0.0, .25000000000000000000E+00, .50000000000000000000E+00,
	    .75000000000000000000E+00, .87500000000000000000E+00, .93750000000000000000E+00, .10000000000000000000E+01,
	    .37500000000000000000E+00, .62500000000000000000E+00, .96875000000000000000E+00, .12500000000000000000E+00,
	    .68750000000000000000E+00, .81250000000000000000E+00, .98437500000000000000E+00, .18750000000000000000E+00,
	    .31250000000000000000E+00, .43750000000000000000E+00, .56250000000000000000E+00, .84375000000000000000E+00,
	    .90625000000000000000E+00, .99218750000000000000E+00 };
    ...
    private static void qxgs(final Function<? super Double, Double> f, final double a, final double b,
	    final double epsabs, final double epsrel, final double[] result, final double[] abserr, final int[] ier,
	    final int limit, final int[] last, final int[] fev) {
	ier[0] = 6;
	last[0] = 0;
    ...
    private static void qxgse1(final Function<? super Double, Double> f, final double a, final double b,
	    final double epsabs, final double epsrel, final int limit, final double[] result, final double[] abserr,
	    final int[] ier, final int[] last, final int[] fev) {
	...
	double area, area12, a1, a2, b1, b2, correc = 0.0, dres, epmach, erlarg = 0.0, erlast, errbnd, erro12, errsum,
		ertest = 0.0, oflow, rerr, small = 0.0, t, uflow;
	final int[] ln1 = new int[1], lp1 = new int[1], ln2 = new int[1], lp2 = new int[1], maxerr = new int[1],
		nrmax = new int[1], numrl2 = new int[1], nres = new int[1];
	final double[] resabs = new double[1], defabs = new double[1], area1 = new double[1], error1 = new double[1],
		defab1 = new double[1], defab2 = new double[1], area2 = new double[1], error2 = new double[1],
		errmax = new double[1], reseps = new double[1], abseps = new double[1];
	...
	ierro = 0;
	lp[1 - 1] = ln[1 - 1] = 1;
	valp[1 - 1][1 - 1] = f.apply((a + b) * 0.5);
	++fev[0];
	valn[1 - 1][1 - 1] = valp[1 - 1][1 - 1];
	qxlqm(f, a, b, result, abserr, resabs, defabs, valp[1 - 1], valn[1 - 1], lp, ln, 2, epmach, uflow, oflow, fev);
	...
	for (last[0] = 2; last[0] <= limit; ++last[0]) {
	    ...
	    qxlqm(f, a1, b1, area1, error1, resabs, defab1, vp1, vn1, lp1, ln1, 2, epmach, uflow, oflow, fev);
	    qxrrd(f, valp[maxerr[0] - 1], lp[maxerr[0] - 1], a2, b2, vp2, vn2, lp2, ln2, fev);
	    qxlqm(f, a2, b2, area2, error2, resabs, defab2, vp2, vn2, lp2, ln2, 2, epmach, uflow, oflow, fev);
		...
		if (Math.abs(rlist[maxerr[0] - 1] - area12) > 0.1e-4 * Math.abs(area12) || erro12 < 0.99 * errmax[0]) {
		} else {
		    if (extrap) {
			++iroff2;
		    }
	...
	if (exit == 110) {
	    ...
	    } else if (0.1e-1 > (result[0] / area) || (result[0] / area) > 0.1e3 || errsum > Math.abs(area)) {
		ier[0] = 6;
	    }
	    if (ier[0] > 2) {
		--ier[0];
    ...
    private static void qxlqm(final Function<? super Double, Double> f, final double a, final double b,
	    final double[] result, final double[] abserr, final double[] resabs, final double[] resasc,
	    final double[] vr, final double[] vs, final int[] lr, final int[] ls, final int key, final double epmach,
	    final double uflow, final double oflow, final int[] fev) {
    ...
    private static void qxrul(final Function<? super Double, Double> f, final double xl, final double xu,
	    final double[] y, final double[] ya, final double[] ym, final int ke, final int k1, final double[] fv1,
	    final double[] fv2, final int[] l1, final int[] l2, final int[] fev) {
    ...
    private static void qxrrd(final Function<? super Double, Double> f, final double[] z, final int lz, final double xl,
	    final double xu, final double[] r, final double[] s, final int[] lr, final int[] ls, final int[] fev) {
	...
	s[7 - 1] = z[1 - 1];
	if (lz <= 11) {
	    r[8 - 1] = f.apply(centr + 0.375 * dlen);
	    r[9 - 1] = f.apply(centr + 0.625 * dlen);
	    r[10 - 1] = f.apply(centr + 0.96875 * dlen);
	    fev[0] += 3;
	    lr[0] = 10;
	    if (lz != 11) {
		s[4 - 1] = f.apply(centr - dlen * 0.75);
		++fev[0];
	    }
	    if (lz == 11) {
		s[4 - 1] = z[11 - 1];
	    }
	    s[5 - 1] = f.apply(centr - dlen * 0.875);
	    s[6 - 1] = f.apply(centr - dlen * 0.9375);
	    s[8 - 1] = f.apply(centr - dlen * 0.375);
	    s[9 - 1] = f.apply(centr - dlen * 0.625);
	    s[10 - 1] = f.apply(centr - dlen * 0.96875);
	    fev[0] += 5;
	    ls[0] = 10;
	    return;
	}
    ...
    private static void qpsrt(final int limit, final int[] last, final int[] maxerr, final double[] ermax,
    ...
    private static void qelg(final int[] n, final double[] epstab, final double[] result, final double[] abserr,
	    final double[] res3la, final int[] nres, final double epmach, final double oflow) {
